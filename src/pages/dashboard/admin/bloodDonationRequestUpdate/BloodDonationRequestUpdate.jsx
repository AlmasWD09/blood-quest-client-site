...
import useSingleDonationRequestData from "../../../../hooks/useSingleDonationRequestData";
...
const BloodDonationRequestUpdate = () => {
    const [singleBloodRequestData,refetch] = useSingleDonationRequestData();
    const { user } = useAuth();
    const {id} = useParams();
    ...
    const {  recipientName,hospitalName, district, upazila, donationDate, donationTime,fullAddress,bloodGroup,description} = singleBloodRequestData || {};
    ...
    const onSubmit = async (data) => {
        ...
        try {
            const { data: updateRequest } = await axiosSecure.put(`/admin/donation/request/api/update/${id}`, updateInfo)
            if (updateRequest?.modifiedCount > 0) {
                toast.success('Donation request updated successfully');
                navigate('/dashboard/all-blood-donation-admin')
            }
        }
        catch (err) {
            console.error(err);
    ...
    return (
        <div className="relative w-full max-w-2xl mx-auto bg-white shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] px-8 mt-10 rounded-lg">
        <form onSubmit={handleSubmit(onSubmit)} className="">
                ...
                <div className="w-full md:w-1/2 mt-3 md:mt-6">
                    ...
                    <div>
                        <select
                            ...
                            className="block w-full py-3 text-gray-700 bg-white border rounded-lg pl-3 dark:bg-gray-900 dark:text-gray-300 dark:border-gray-600 focus:border-primary dark:focus:border-blue-300 focus:ring-blue-300 focus:outline-none focus:ring focus:ring-opacity-40">
                            <option disabled value={'Choose your blood group'}>Choose your blood group</option>
                            {
            ...
            <div className="mt-3 md:mt-6">
                ...
                <textarea
                    ...
                    className='resize-none w-full px-4 py-6 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-primary focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring' placeholder="Request Message..." />
            </div>

            {/* update request button */}
            <div className="flex justify-end py-6">
                <button type="submit" className="disabled:bg-gray-200  disabled:cursor-not-allowed px-6 py-3 text-sm font-medium tracking-wide text-white capitalize transition-colors duration-300 transform bg-primary rounded-lg ">
                    Update Request
                </button>
            </div>
        </form>
...
export default BloodDonationRequestUpdate;